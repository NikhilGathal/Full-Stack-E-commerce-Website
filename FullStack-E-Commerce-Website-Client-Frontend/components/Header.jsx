...
export default function Header({
  ...
  function handleDeleteAccount() {
    ...
    if (!currentUsername) {
      ...
      return
    } // Step 1: Get user details from backend

    fetch(`http://localhost:8080/api/users/${currentUsername}`)
      .then((res) => {
        if (!res.ok) {
      ...
      .then((user) => {
        const userEmail = user.email // Step 2: Delete the user account

        return fetch(
          `http://localhost:8080/api/users/username/${currentUsername}`,
          {
            method: 'DELETE',
          }
        )
          .then((res) => {
            ...
            } // Step 3: Delete the email from subscriptions

            return fetch(
              `http://localhost:8080/api/subscriptions/${userEmail}`,
              {
                method: 'DELETE',
              }
            )
          ...
          .then(() => {
            alert('Account and subscription deleted successfully')
      ...
      .catch((error) => {
        ...
        alert('Failed to delete account or subscription')
      })
  }
